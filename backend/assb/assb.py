...
def assb_bot():
    ...
    try:
        ...
        if not exchange.apiKey or not exchange.secret:
            logging.info("API credentials not found. Skipping this iteration.")
            return

        # Fetch market data
        for symbol in symbols:
          market_data = fetch_market_data(symbol)
          logging.info("Market data", market_data.__str__())
          if market_data.empty:
            logging.info("No market data available. Skipping this iteration.")
            return
        ...
        # Calculate sentiment
         ...
         # Execute trade
          if decision == "BUY":
            place_trade(symbol, "BUY", amount=amount)
          elif decision == "SELL":
            place_trade(symbol, "SELL", amount=amount)
          else:
              logging.info("No trade signal.")
    except Exception as e:
        logging.info(f"Error in bot execution: {e}")
