...
const PrivacyPolicy: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPolicies = async () => {
      try {
        const termsResponse = await axiosPublic.get<string[]>(
          "[Your Privacy Policy API Endpoint]",
        );
        const securityResponse = await axiosPublic.get<string[]>(
          "[Your Data Security Policy API Endpoint]",
        );
        const cookieResponse = await axiosPublic.get<string>(
        ...
        setError(
          err.response.message || "An error occurred while fetching policies.",
        );
      }
    };
