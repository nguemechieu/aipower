...
const StellarAccount = () => {
  ...
  useEffect(() => {
    const fetchAccountData = async () => {
      try {
        ...
        setError(error.response.data); // Handle error in case of failure
      }
    };
  ...
  return (
    <div className="stellar-account">
      ...
      <table>
        <tbody>
          ...
          <tr>
            <th>Thresholds</th>
            <td>
              Low: {account.AccountResponse.thresholds.lowThreshold}, Medium:{" "}
              {account.AccountResponse.thresholds.medThreshold}, High:{" "}
              {account.AccountResponse.thresholds.highThreshold}
            </td>
          </tr>
          <tr>
            <th>Flags</th>
            <td>
              Auth Required:{" "}
              {account.AccountResponse.flags.authRequired ? "Yes" : "No"},<br />
              Auth Revocable:{" "}
              {account.AccountResponse.flags.authRevocable ? "Yes" : "No"}
            </td>
          </tr>
          <tr>
            <th>Balances</th>
            <td>
              {account.AccountResponse.balances &&
                account.AccountResponse.balances.map((balance, index) => (
                  <div key={index}>
      ...
      <h3>Signers</h3>
      <ul>
        {account.AccountResponse.signers &&
          account.AccountResponse.signers.map((signer, index) => (
            <li key={index}>
              <p>Key: {signer.key}</p>
      ...
      {account.AccountResponse.links && (
        <ul>
          ...
          <li>
            <a href={account.AccountResponse.links.transactions?.href}>
              Transactions
            </a>
          </li>
          <li>
            <a href={account.AccountResponse.links.operations?.href}>
              Operations
            </a>
          </li>
          <li>
