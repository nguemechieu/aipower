...
const GitHubRedirect = () => {
  ...
  useEffect(() => {
    const handleOAuthCallback = async (event: { data: string | Record<string, string> | string[][] | URLSearchParams | undefined; }) => {
      try {
        ...
        if (!code) {
          setTimeout(() => {
            navigate("/login"); // Redirect to log in if authentication fails
          }, 2000);
