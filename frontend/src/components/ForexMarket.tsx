  ...
  useEffect(() => {
    const fetchForexData = async () => {
      ...
      try {
        ...
        const forexInstruments = response.data.instruments.filter(
          (instrument: { type: string; }) => instrument.type === "CURRENCY",
        );

        const forexQuotes :any= await Promise.all(
          forexInstruments.map(async (instrument: { name: any; displayName: any; }) => {
        ...
        setForexData(forexQuotes.response.data);
      } catch (error) {
